...
local ftplugin_dir = lsp_cfg.templates_dir
...
function M.generate_ftplugin(server_name, dir)
  ...
  if vim.tbl_contains(lsp_cfg.override, server_name) and not has_custom_provider then
    return
  end

  -- we need to go through lspconfig to get the corresponding filetypes currently
  local filetypes = myvim_lsp_utils.get_supported_filetypes(server_name) or {}
...
function M.generate_templates(servers_names)
  ...
  if not utils.is_directory(lsp_cfg.templates_dir) then
